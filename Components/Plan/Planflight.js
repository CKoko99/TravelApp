...
function Planflight() {
  ...
  const flightList = flights.map((item) => {
    ...
    return <>
  {selectedFlight && <Pagecard desc={selectedFlight.desc} rating={selectedFlight.rating} title={selectedFlight.title} img={selectedFlight.imgs[0]} />}
  {selectedFlight && <button onClick={selectFlightHandler}>Choose a Hotel</button>}
  {flightList}</>;
}
