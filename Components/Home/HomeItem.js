...
function HomeItem(props) {
  ...
  for (let i = 1; i <= props.rating; i++) {
    filledStars.push(<Image width={15} height={15} className={classes.star} key={i} alt="ratings" src={Star} />);
  }
  for (let i = 1; i <= EmptyStarAmount; i++) {
    EmptyStars.push(<Image width={15} height={15} key={i} className={classes.star} alt="ratings" src={EmptyStar} />);
  }}else{
    rating = "New Listing"
  }
  return (
